...
namespace Hl7.Fhir.FhirPath.Validator
{
	public class SearchExpressionValidator
	{
		...
		public Func<BaseFhirPathExpressionVisitor> CreateFhirPathValidator { get; set; }
		...
		public IEnumerable<OperationOutcome.IssueComponent> Validate(string type, string code, string expression, SearchParamType searchType, string url, VersionAgnosticSearchParameter spd)
		{
			...
			BaseFhirPathExpressionVisitor visitor;
			if (CreateFhirPathValidator == null)
				visitor = new BaseFhirPathExpressionVisitor(_mi, _supportedResources, _openTypes);
			else
				visitor = CreateFhirPathValidator();
			...
			foreach (var returnType in r.ToString().Replace("[]", "").Replace(" ", "").Split(','))
			{
				switch (searchType)
				{
					...
					case SearchParamType.Composite:
						...
						if (spd == null)
						{
							// There was no SPD to process with!
							var issue = new Hl7.Fhir.Model.OperationOutcome.IssueComponent()
							{
								Severity = Hl7.Fhir.Model.OperationOutcome.IssueSeverity.Error,
								Code = Hl7.Fhir.Model.OperationOutcome.IssueType.Exception,
								Details = new Hl7.Fhir.Model.CodeableConcept(SearchTypeMismatch.System, SearchTypeMismatch.Code, SearchTypeMismatch.Display, "Composite without a Search parameter definition"),
						...
						else
						{
							...
							foreach (var cp in spd.Component)
							{
								...
								{
									// At present can't handle composites of composites
									if (componentSearchParameter.Type == SearchParamType.Composite)
									{
										var message = $"Cannot validate composite search parameters with a composite component `{componentSearchParameter.Code}`: {cp.Definition} in {spd.Url}";
										var issue = new Hl7.Fhir.Model.OperationOutcome.IssueComponent()
										{
											Severity = Hl7.Fhir.Model.OperationOutcome.IssueSeverity.Error,
											Code = Hl7.Fhir.Model.OperationOutcome.IssueType.Exception,
									...
									else
									{
										...
										{
											BaseFhirPathExpressionVisitor visitorComponent;
											if (CreateFhirPathValidator == null)
												visitorComponent = new BaseFhirPathExpressionVisitor(_mi, _supportedResources, _openTypes);
											else
												visitorComponent = CreateFhirPathValidator();
		...
		readonly string[] UriTypes = {
			...
			"oid",
			"uuid",
			// Gino clarified again, that this is not actually valid, changed the IG that was doing it
